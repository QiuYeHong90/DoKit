...
- (void)setLeftNavClose{
    DoraemonNavBarItemModel *item = [[DoraemonNavBarItemModel alloc] initWithText:@"Close" color:[UIColor doraemon_blue] selector:@selector(closeBtnClick)];
    NSArray *barItems = [self navigationItems:@[item]];
    if (barItems) {
        self.navigationItem.leftBarButtonItems = barItems;
    }
