...
@property (nonatomic, strong) DoraemonRTMObserveWindow *rtmWindow;
...
- (instancetype)init{
    if (self = [super init]) {
        ...
        _rtmWindow = [DoraemonRTMObserveWindow shareInstance];
    }
    return self;
}
...
- (void)resetLayout{
    ...
    if (!_rtmWindow.hidden) {
        _rtmWindow.frame = CGRectMake(0, offsetY, width, height);
    }
}
