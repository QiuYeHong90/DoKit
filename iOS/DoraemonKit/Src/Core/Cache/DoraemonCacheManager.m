...
static NSString * const kDoraemonRtmKey = @"kDoraemonRtmKey";
...
- (void)saveRTMObserveSwitch:(BOOL)on{
    [_defaults setBool:on forKey:kDoraemonRtmKey];
    [_defaults synchronize];
}

- (BOOL)rtmSwitch{
    return [_defaults boolForKey:kDoraemonRtmKey];
}
- (BOOL)fpsSwitch{
